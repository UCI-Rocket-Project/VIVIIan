...
@unittest.skipIf(SharedRingBuffer is None or SharedMemorySpec is None, "shared_ring_buffer dependencies are required")
    ...
    def test_shared_memory_spec_rejects_invalid_size_and_reader_count(self) -> None:
        ...
        for size, num_readers in cases:
            with self.subTest(size=size, num_readers=num_readers):
                with self.assertRaises(ValueError):
                    SharedMemorySpec(name="spec", size=size, num_readers=num_readers)
