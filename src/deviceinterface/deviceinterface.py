...
class ArrowBatchStreamServer:
    ...
    def __init__(self, host: str, port: int) -> None:
        ...
        self._metadata_client: socket.socket | None = None
        logger.info("Arrow stream server listening on %s:%s", host, port)
        logger.info("Metadata server listening on %s:%s", host, port + 1)
    ...
    def _try_accept_data(self) -> None:
        ...
        try:
            ...
            logger.info("Arrow stream client connected: %s", addr)
    ...
    def _try_accept_metadata(self) -> None:
        ...
        try:
            ...
            logger.info("Metadata client connected: %s", addr)
        except BlockingIOError:
            return
...
class DeviceInterface:
    ...
    def _chunk_table(self, table: pa.Table) -> Iterator[pa.Table]:
        chunk_size = max(1, int(self.max_rows))
        for start in range(0, table.num_rows, chunk_size):
            yield table.slice(start, chunk_size)
    ...
    def _tx_table(self) -> None:
        ...
        for chunk_table in self._chunk_table(concat_table):
            self.stream_server.send_table(chunk_table)
            with self._lock:
    ...
    def _sender_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            has_maxrows = queued_rows >= max(1, int(self.max_rows))
