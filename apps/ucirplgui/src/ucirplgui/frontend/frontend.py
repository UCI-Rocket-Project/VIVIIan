...
def run_frontend() -> None:
    ...
    def feed_loop() -> None:
        while True:
            link_store.update(read_device_link_snapshots())
            ...
            if rx_scalars.has_batch:
                ...
                readers["eng_tc_1"].prime(float(row[0]), float(row[1]))
                readers["eng_tc_2"].prime(float(row[0]), float(row[2]))
                readers["gn2_chamber_proxy"].prime(float(row[0]), float(row[3]))
                readers["copv_tc"].prime(float(row[0]), float(row[4]))
            time.sleep(0.02)
