...
- timestamps that are part of the data contract
...
- assigning authoritative acquisition timestamps
...
Internally, the backend should prefer `pythusa` for local stream movement and processing DAG execution when high-rate paths matter.
...
VIVIIan intentionally does not standardize separate connector classes like "data connector," "command connector," and "health connector" as distinct architecture-level primitives.
Instead, there is one generic connector abstraction with different schema-bound uses.
...
All inter-unit traffic should move through connectors that are explicit about these properties.

### StreamSpec

`StreamSpec` defines a versioned schema contract for telemetry or state payloads.
...
`CommandSpec` defines a versioned schema contract for control payloads.

Commands in VIVIIan are:

- typed
- versioned
- one-way
- generally desired-state or setpoint oriented

The architecture does not require semantic command acknowledgements.
A command is a typed Arrow table sent from the frontend to the device interface.
The device interface interprets it and later emits updated state through normal telemetry.

This is a major design choice:

- the command path stays simple
- transport stays uniform
- device-specific semantics remain device-local
- frontend correctness depends on observed state, not on bespoke reply messages
...
Device acquisition timestamps are authoritative.

That means:

- the device interface stamps data as close to acquisition as practical
- downstream systems preserve those timestamps end to end
- the backend may add additional ingest or processing timestamps as secondary metadata
- downstream logic must not silently replace acquisition time with receive time
...
- storage correctness
- operator trust in plots and state transitions

Backend receive time is operationally useful, but it is not the canonical clock for measured data.
...
2. `deviceinterface` converts them into typed versioned payloads with authoritative timestamps
...
1. `frontend` emits a typed command/setpoint Arrow table
...
- stream schemas and command schemas should be designed with bounded, freshest-wins behavior in mind
...
- device-authoritative timestamps
